...
impl AppPaths {
    pub fn discover() -> Result<Self> {
        ...
        let app_support = macos_application_support_dir(&home);
        let config_root = env::var_os("XDG_CONFIG_HOME")
            .map(PathBuf::from)
            .filter(|path| path.is_absolute())
            .unwrap_or_else(|| app_support.clone())
            .join("claude-account");

        let data_root = env::var_os("XDG_DATA_HOME")
            .map(PathBuf::from)
            .filter(|path| path.is_absolute())
            .unwrap_or(app_support)
            .join("claude-account");
...
fn macos_application_support_dir(home: &Path) -> PathBuf {
...
mod tests {
    ...
    fn defaults_to_macos_application_support() {
        assert_eq!(
            macos_application_support_dir(Path::new("/Users/example")),
